...
contract AyalaENS is Ownable {

    constructor() {}

    mapping(string => string) public ENS; // Name ---> Address + tokenID  


    function updateENS(string memory _identityNumber, string memory _wantedName) public {
        require(bytes(ENS[toLowerCase(_wantedName)]).length == 0, "Name is already taken"); // Uniqueness of the wanted name
        // ends with .pol for polygon
        require(checkOwnership(_identityNumber) == msg.sender, "Not a valid owner"); // Only Owner can change it.

        // Is the wantedName needs to be aswell in the lower case in order to to preserver consistity?
        ENS[toLowerCase(_wantedName)] = toLowerCase(_identityNumber);
    }
